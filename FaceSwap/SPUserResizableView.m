...
#define kSPUserResizableViewDefaultMinWidth 25.0
#define kSPUserResizableViewDefaultMinHeight 25.0
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    /*
    CGRect lastRect = CGRectMake(self.frame.origin.x + self.frame.size.width - 10.0 , self.frame.origin.y + self.frame.size.height - 10, 20, 20);
    NSLog(@"lastRect: %@",NSStringFromCGRect(lastRect));
    if (CGRectContainsPoint(lastRect, touchStart) == true){
        NSLog(@"YES: At last");
        shouldResize = false;
    }else{
        NSLog(@"NO: somewhere else");
        shouldResize = true;
    }*/
    shouldResize = true;
    
//    NSLog(@"touchStart: %@ frame: %@", NSStringFromCGPoint(touchStart), NSStringFromCGRect(self.frame));
    if (![self isResizing]) {
...
- (void)resizeUsingTouchLocation:(CGPoint)touchPoint {
    ...
    /** TRYING TO RESIZE MASK BUT NO LUCK
     CGRect rectSuper = self.superview.frame;
     CGRect rectSelf = self.frame;
     CGRect rectSelfNew = CGRectMake(newX, newY, newWidth, newHeight);
     
     CGFloat diffY = rectSelf.origin.y - rectSelfNew.origin.y;
     CGFloat diffX = rectSelf.origin.x - rectSelfNew.origin.x;
     CGFloat diffHeight = rectSelf.size.height - rectSelfNew.size.height;
     CGFloat diffWidth = rectSelf.size.width - rectSelfNew.size.width;
     
     rectSuper.origin.y =  rectSuper.origin.y - diffY;
     rectSuper.origin.x =  rectSuper.origin.x - diffX;
     rectSuper.size.height = diffHeight;
     rectSuper.size.width = diffWidth;
     */
    //self.frame = CGRectMake(newX, newY, newWidth, newHeight);

    
    /** with bound property
    CGFloat newX = self.bounds.origin.x + deltaX;
    CGFloat newY = self.bounds.origin.y + deltaY;
    ...
    if (self.preventsPositionOutsideSuperview) {
        if (newX < self.superview.bounds.origin.x) {
            // Calculate how much to grow the width by such that the new X coordintae will align with the superview.
            deltaW = self.bounds.origin.x - self.superview.bounds.origin.x;
            newWidth = self.bounds.size.width + deltaW;
            newX = self.superview.bounds.origin.x;
        }
        if (newX + newWidth > self.superview.bounds.origin.x + self.superview.bounds.size.width) {
            newWidth = self.superview.bounds.size.width - newX;
        ...
        if (newY < self.superview.bounds.origin.y) {
            ...
            newHeight = self.bounds.size.height + deltaH;
            newY = self.superview.bounds.origin.y;
        }
        if (newY + newHeight > self.superview.bounds.origin.y + self.superview.bounds.size.height) {
            newHeight = self.superview.bounds.size.height - newY;
...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event {
    if (shouldResize) {
        ...
        CGFloat angle = atan2f(target.y-position.y, target.x - position.x);
